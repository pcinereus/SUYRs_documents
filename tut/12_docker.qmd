...
echo '' >> Dockerfile4
echo '## Default command parameters' >> Dockerfile4
echo 'CMD ["/home/Project/analysis.R"]' >> Dockerfile4
```
...
#| highlight-style: zenburn
cd ~/tmp/docker_tests/
docker run --privileged --entrypoint Rscript --rm -v $(pwd):/home/Project r:2 -e "Sys.info()['user']; list.files(); getwd(); list.files('/'); list.files('~'); list.files('/home');list.files('/home/Project');list.files('/workspace');list.files('/home/root/');"
```
